...
var TT = { 
	...
	, startAct: function (toHref, fromHref, back) {
		...
		TT.smoothPg.dispatch({typ:TT.PRE, toHref:toHref, fromHref:fromHref, $new:toHref, back:back}) //$new is for backward compatibility
	}

	, actReady: function ($newContent, $html, toHref, fromHref, back) {
		var delta = new Date().getTime() - TT._actStarted
		TT.smoothPg.dispatch({typ:TT.PAGE, toHref:toHref, fromHref:fromHref, $new:$newContent, delta:delta, $html:$html, back:back})
		TT.inAction=false
	}//()
	...
	, loadPg: function(toHref, fromHref, back) {//triggered, but function can be called directly also
